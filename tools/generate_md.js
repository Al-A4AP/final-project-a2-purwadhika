...
const printReport = (candidates) => {
  let md = '| No | File | Baris | Fungsi/Component | Kind | Jumlah Baris |\n';
  md += '| --- | --- | ---: | --- | --- | ---: |\n';
  candidates.forEach((c, i) => {
    md += `| ${i + 1} | \`${c.path}\` | ${c.line} | \`${c.name}\` | ${c.kind} | ${c.lines} |\n`;
  });

  const mdFile = require('path').join(ROOT_DIR, 'docs/audits/AUDIT_CLEAN_CODE_REST_API_GUIDELINES.md');
  let content = fs.readFileSync(mdFile, 'utf8');

  const regex = /\| No \| File \| Baris \| Fungsi\/Component \| Kind \| Jumlah Baris \|[\s\S]*?\*\(Daftar di atas menampilkan 25 teratas dari total 103 kandidat\. Jalankan \`npm run audit:functions\` untuk melihat seluruhnya\.\)\*/;
  ...
  content = content.replace(regex, md);
  fs.writeFileSync(mdFile, content);
  console.log("Updated MD file with all " + candidates.length + " candidates");
};
