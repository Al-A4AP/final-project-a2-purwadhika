...
const errorMessage = (err: any) => err?.issues?.[0]?.message || err.message || 'Terjadi kesalahan internal';
const statusCode = (err: any) => err?.statusCode || (err?.issues ? 400 : 500);
const handleError = (res: Response, err: any) => sendError(res, errorMessage(err), statusCode(err));
