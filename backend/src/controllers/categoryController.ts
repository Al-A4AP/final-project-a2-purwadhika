...
const errorMessage = (err: ApiError) =>
  err?.issues?.[0]?.message || err.message || "Terjadi kesalahan internal";
const statusCode = (err: ApiError) =>
  err?.statusCode || (err?.issues ? 400 : 500);
const handleError = (res: Response, err: unknown) =>
  sendError(res, errorMessage(err as ApiError), statusCode(err as ApiError));
