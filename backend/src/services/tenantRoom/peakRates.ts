...
const ensureNoPeakRateOverlap = async (roomId: string, start: Date, end: Date) => {
  const existing = await findOverlappingPeakRate(roomId, start, end);
  if (existing) throw new AppError('Terdapat jadwal Peak Season yang bentrok pada rentang tanggal tersebut', 400);
};
const findOverlappingPeakRate = (roomId: string, start: Date, end: Date) => prisma.peakSeasonRate.findFirst({
  where: { roomId, deleted_at: null, start_date: { lte: end }, end_date: { gte: start } },
});
