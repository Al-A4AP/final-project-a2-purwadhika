...
export const getTenantProperties = async (tenantId: string, options: GetTenantPropertiesOptions = {}) => {
  ...
  return { properties, pagination: buildPagination(normalized.page, normalized.limit, total) };
};
...
export const addPropertyImage = (propertyId: string, tenantId: string, file: Express.Multer.File) =>
