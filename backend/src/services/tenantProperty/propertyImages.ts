...
import { deleteFromCloudinary, uploadBuffer } from '../../utils/cloudinaryUpload';
import { findTenantProperty } from './tenantPropertyQueries';

export const createPropertyImage = async (propertyId: string, tenantId: string, file: Express.Multer.File) => {
  await ensurePropertyExists(propertyId, tenantId);
  const result = await uploadBuffer(file.buffer);
  const order = await getNextImageOrder(propertyId);
  return prisma.propertyImage.create({
    data: { propertyId, image_url: result.url, cloudinary_public_id: result.public_id, order },
  });
};

export const removePropertyImage = async (propertyId: string, imageId: string, tenantId: string) => {
  const deleted = await prisma.$transaction((tx) => deleteImageRecord(tx, propertyId, imageId, tenantId));
...
const deleteImageRecord = async (tx: Prisma.TransactionClient, propertyId: string, imageId: string, tenantId: string) => {
...
const ensureTransactionProperty = async (tx: Prisma.TransactionClient, propertyId: string, tenantId: string) => {
  const property = await tx.property.findFirst({ where: { id: propertyId, tenantId } });
  if (!property) throw new AppError('Properti tidak ditemukan', 404);
};
const ensureTransactionImage = async (tx: Prisma.TransactionClient, propertyId: string, imageId: string) => {
...
const deleteCloudinaryImage = async (publicId?: string | null) => {
  if (!publicId) return;
  try { await deleteFromCloudinary(publicId); } catch { return; }
};
