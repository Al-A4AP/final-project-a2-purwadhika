...
export const useBookingPageState = (): BookingPageState => {
  ...
  const checkout = useBookingCheckout({ agreementAccepted, query, ...data, ...guestIdentity, ...guests, ...voucher, paymentMethod, navigate });
  return { ...data, agreementAccepted, dateForm, ...guestIdentity, ...guests, ...voucher, paymentMethod, setAgreementAccepted, setPaymentMethod, ...checkout, totals };
};
