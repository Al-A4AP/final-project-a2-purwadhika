...
const useUserOrderFilters = () => {
  ...
  const values = useMemo<UserOrderFilters>(
    () => ({
      checkInDate,
      checkOutDate,
      orderNumber,
    ...
    [checkInDate, checkOutDate, orderNumber, status, sortOrder],
  );

  const actions = useMemo<UserOrderFilterActions>(
    () => ({
      resetFilters,
      setCheckInDate,
      setCheckOutDate,
...
const useFetchUserOrders = (
  ...
  useCallback(
    (page = 1) => {
      ...
      setError(null);
      orderService
        .getUserOrders(buildUserOrderParams(filters, page))
        .then((data) => {
