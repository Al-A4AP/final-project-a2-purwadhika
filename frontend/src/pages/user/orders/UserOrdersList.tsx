...
export const UserOrdersList: FC<{ state: UserOrdersState }> = ({ state }) => {
  ...
  return <div className="space-y-4">{state.orders.map((order) => <OrderCard key={order.id} order={order} uploading={state.uploading} handleUploadClick={state.handleUploadClick} canceling={state.canceling} handleCancelClick={state.handleCancelClick} paymentActionId={state.paymentActionId} retryMidtransPayment={state.retryMidtransPayment} switchToManualPayment={state.switchToManualPayment} reviewOrderId={state.reviewOrderId} setReviewOrderId={state.setReviewOrderId} rating={state.rating} setRating={state.setRating} comment={state.comment} setComment={state.setComment} handleReviewSubmit={state.handleReviewSubmit} submittingReview={state.submittingReview} StatusBadge={UserOrderStatusBadge} />)}<Pagination currentPage={state.pagination.page || 1} totalPages={totalPages} totalItems={state.pagination.total} onPageChange={state.fetchOrders} /></div>;
};
