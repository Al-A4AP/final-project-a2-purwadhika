...
import type {
  UserOrderFilterActions,
  UserOrderFilters,
} from "@/hooks/user/orders/userOrdersTypes";
...
const FIELD_ICON_CLASS =
  "pointer-events-none absolute left-4 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-slate-400";
const INPUT_CLASS =
  "h-12 w-full rounded-xl border-none bg-slate-50 py-3 pl-10 pr-4 text-sm font-medium outline-none transition-all focus:ring-2 focus:ring-red-500 dark:bg-slate-800 dark:text-white";

export const UserOrdersFilter: FC<UserOrdersFilterProps> = ({
  actions,
  filters,
  onSearch,
}) => {
...
const FilterFields: FC<FilterFieldsProps> = ({ actions, filters }) => (
  <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4">
    ...
    <SortOrderField value={filters.sortOrder} onChange={actions.setSortOrder} />
    <DateField
      label="Check-in"
...
const SortOrderField: FC<SortOrderFieldProps> = ({ onChange, value }) => (
...
const DateField: FC<TextFieldProps & { label: string }> = ({
  label,
  onChange,
  value,
}) => (
...
const FilterButtons: FC<FilterButtonsProps> = ({
  ...
  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
    <button
      onClick={onSearch}
      className="h-12 rounded-xl bg-red-600 px-8 text-sm font-bold text-white shadow-sm transition hover:bg-red-700 hover:shadow-md md:col-span-2"
    >
      Terapkan Filter
    </button>
    <button
      ...
      className="h-12 rounded-xl bg-slate-100 px-8 text-sm font-bold text-slate-700 transition hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700 md:col-span-2"
    >
      Reset Semua
    </button>
  </div>
