...
export const UserOrdersContent: FC<UserOrdersContentProps> = ({
  fileInputRef,
  state,
}) => {
  ...
  if (state.loading && state.orders.length === 0) {
    return (
      <SectionLoading
        ...
        label="Memuat riwayat pemesanan..."
        size="md"
        variant="cards"
      />
    );
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 max-w-xl">
                Lacak status reservasi Anda, unggah bukti pembayaran, atau
                berikan ulasan untuk tempat yang sudah Anda inapi.
              </p>
            </div>
            {totalOrders > 0 && (
