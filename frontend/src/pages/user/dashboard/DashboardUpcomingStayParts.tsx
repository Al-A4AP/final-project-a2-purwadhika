...
export const EmptyUpcomingStay: FC = () => (
  <div className="mb-8">
...
const ExploreLink: FC = () => (
  <Link to="/explore" className="rounded-xl bg-red-600 px-6 py-3 font-semibold text-white transition hover:bg-red-700">
    Eksplorasi Properti
  </Link>
);

export const UpcomingStayCard: FC<UpcomingStayCardProps> = ({ checkIn, checkOut, order }) => (
  <div className="mb-8">
    <h2 className="mb-4 text-xl font-bold text-slate-900 dark:text-white">Reservasi Terkonfirmasi Terbaru</h2>
    <div className="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm transition-all hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col md:flex-row">
        ...
        <UpcomingStayDetails checkIn={checkIn} checkOut={checkOut} order={order} />
      </div>
    </div>
  </div>
);

const PropertyImage: FC<{ order: Order }> = ({ order }) => (
...
const UpcomingStayDetails: FC<UpcomingStayCardProps> = ({ checkIn, checkOut, order }) => (
  <div className="flex flex-1 flex-col justify-between p-6 md:p-8">
    <div>
      <UpcomingStayHeader order={order} />
      <StayDateRange checkIn={checkIn} checkOut={checkOut} />
    </div>
    <UpcomingStayFooter order={order} />
  </div>
);

const UpcomingStayHeader: FC<{ order: Order }> = ({ order }) => (
  <>
...
const StayDateRange: FC<Pick<UpcomingStayCardProps, "checkIn" | "checkOut">> = ({ checkIn, checkOut }) => (
...
const UpcomingStayFooter: FC<{ order: Order }> = ({ order }) => (
