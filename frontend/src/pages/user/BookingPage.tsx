...
const BookingPage: FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"MANUAL" | "MIDTRANS">(
    "MIDTRANS",
  );
  ...
  const updateGuest = (
    ...
    setGuests((prev) => {
      ...
      if (type === "adults") {
        ...
        nextChildren = Math.max(
          0,
          Math.min(prev.adults, prev.children + delta),
        );
      } else if (type === "babies") {
  ...
  useEffect(() => {
    ...
    propertyService
      .getPropertyDetail(propertyId, checkIn, checkOut)
      .then((data) => {
        setProperty(data);
  ...
  const totalRoomPrice = room.priceDetails
    ? room.priceDetails.totalPrice
    : room.base_price * nights;
  ...
  const handleCheckout = async () => {
    ...
    const checkInDateCompare = new Date(checkIn! + "T00:00:00Z");
    ...
    if (guests.adults > room.capacity) {
      toast.error(
        `Jumlah orang dewasa tidak boleh melebihi kapasitas kamar (${room.capacity} orang).`,
      );
      return;
    }
    ...
    try {
      const result = await orderService.createOrder({
        ...
        roomId: room.id,
        check_in_date: checkIn! + "T00:00:00Z",
        check_out_date: checkOut! + "T00:00:00Z",
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border dark:border-slate-700">
            ...
            <div className="space-y-4">
                ...
                [
                  {
                    key: "adults",
                    label: "Dewasa",
                    desc: "Usia 13 tahun ke atas",
                  },
                  {
                    key: "children",
                    label: "Anak-anak",
                    desc: "Usia 2 - 12 tahun",
                  },
              ...
              <div className="text-xs text-gray-400 border-t dark:border-slate-700 pt-3 flex flex-col sm:flex-row justify-between gap-2">
                <span>
                  Total tamu: <strong>{totalGuests} orang</strong>
                  {guests.babies > 0 ? ` + ${guests.babies} bayi` : ""}
                </span>
                <span className="text-gray-500">
                  Kapasitas kamar: <strong>{room.capacity} Dewasa</strong>{" "}
                  (Maks. Anak & Bayi sesuai jumlah Dewasa)
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border dark:border-slate-700">
            ...
            <div className="space-y-3">
              <label
                className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition ${paymentMethod === "MIDTRANS" ? "border-red-600 bg-red-50 dark:bg-red-900/10" : "border-gray-200 dark:border-slate-700"}`}
              >
                <input
                  type="radio"
                  name="payment"
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Kartu Kredit, GoPay, BCA Virtual Account, dll.
                  </p>
                </div>
              </label>

              <label
                className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition ${paymentMethod === "MANUAL" ? "border-red-600 bg-red-50 dark:bg-red-900/10" : "border-gray-200 dark:border-slate-700"}`}
              >
                <input
                  type="radio"
                  name="payment"
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Transfer ke rekening bank dan unggah bukti pembayaran.
                  </p>
                </div>
              </label>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border dark:border-slate-700 sticky top-24">
            ...
            <div className="space-y-3 text-xs mb-6 border-y dark:border-slate-700 py-4 max-h-48 overflow-y-auto">
              <div className="font-semibold text-gray-700 dark:text-gray-300">
                Sewa Kamar ({nights} malam):
              </div>
              {room.priceDetails ? (
                room.priceDetails.breakdown.map((day, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between text-gray-600 dark:text-gray-400 pl-2"
                  >
                ...
                <div className="flex justify-between pl-2">
                  <span className="text-gray-600 dark:text-gray-400">
                    {formatPrice(room.base_price)} x {nights} malam
                  </span>
                  <span className="text-gray-905 dark:text-white font-medium font-mono">
