...
const PropertyDetailPage: FC = () => {
  ...
  const checkInUTC = checkIn ? formatDateToUTC(checkIn) : "";
  ...
  const handleBooking = (room: Room) => {
    ...
    if (!checkIn || !checkOut) {
      ...
      document
        .getElementById("date-picker-section")
        ?.scrollIntoView({ behavior: "smooth", block: "center" });
      return;
    }
    const checkInUTC = formatDateToUTC(checkIn);
    const todayStr = new Date().toISOString().split("T")[0];
    if (checkInUTC < todayStr) {
      setDateError("Tanggal check-in tidak boleh di masa lalu.");
      return;
    }
    const checkOutUTC = formatDateToUTC(checkOut);
    ...
    navigate(
      `/booking?propertyId=${id}&roomId=${room.id}&checkIn=${checkInUTC}&checkOut=${checkOutUTC}`,
    );
  };
  ...
  const handleCheckAvail = async (room: Room) => {
    ...
    try {
      ...
      setBlockedDays(
        data.filter((a) => !a.is_available).map((a) => new Date(a.date)),
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-20">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-gray-600 dark:text-gray-400 mb-6 hover:text-red-600"
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={checkOut}
                  min={checkInUTC || new Date().toISOString().split("T")[0]}
                  onChange={(e) => {
        ...
        {(() => {
          const isWholeUnit = ["Villa", "Rumah"].includes(
            property.category?.name || "",
          );
          const firstRoom = property.rooms?.[0];

          if (isWholeUnit && firstRoom) {
            const firstRoomPrice = firstRoom.priceDetails
              ? firstRoom.priceDetails.totalPrice
              : firstRoom.base_price;
            return (
              <div className="bg-white dark:bg-slate-800 rounded-xl p-8 shadow-sm border dark:border-slate-700">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">
                  Anda akan menyewa seluruh{" "}
                  {property.category?.name?.toLowerCase()} ini secara eksklusif.
                </p>
                <div className="flex flex-wrap gap-6 items-center justify-between">
                  <div className="space-y-1">
                    <div className="flex gap-6 text-sm text-gray-600 dark:text-gray-400">
                      <span className="flex items-center gap-1">
                        <BedDouble size={16} /> Kapasitas: {firstRoom.capacity}{" "}
                        orang
                      </span>
                    </div>
                  ...
                  <div className="text-right">
                    {firstRoomPrice === 0 ? (
                      ...
                      <p className="text-3xl font-bold text-red-600">
                        {formatPrice(firstRoomPrice)}
                      </p>
                    )}
                    <p className="text-sm text-gray-500">
                      {firstRoom.priceDetails
                        ? `total (${firstRoom.priceDetails.nights} malam)`
                        : "/ malam (seluruh unit)"}
                    </p>
                    {firstRoom.priceDetails &&
                      firstRoom.priceDetails.nights > 1 &&
                      firstRoomPrice > 0 && (
                        <p className="text-xs text-gray-400 mt-1">
                          Rata-rata:{" "}
                          {formatPrice(
                            Math.round(
                              firstRoomPrice / firstRoom.priceDetails.nights,
                            ),
                          )}{" "}
                          / malam
                        </p>
                ...
                {firstRoom.priceDetails && (
                  <div className="mt-6 text-sm bg-gray-50 dark:bg-slate-700/30 p-4 rounded-lg space-y-1 border dark:border-slate-750">
                    <p className="font-semibold text-gray-700 dark:text-gray-300 mb-2">
                      Rincian Harga Menginap:
                    </p>
                    {firstRoom.priceDetails.breakdown.map((day, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between text-gray-600 dark:text-gray-400 py-1 border-b border-gray-100 dark:border-slate-700/50 last:border-0"
                      >
            ...
            <>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {property.rooms?.map((room) => {
                  const roomPrice = room.priceDetails
                    ? room.priceDetails.totalPrice
                    : room.base_price;
                  return (
                    <div
                      key={room.id}
                      className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border dark:border-slate-700 flex flex-col"
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex gap-4 text-sm text-gray-600 dark:text-gray-400 mb-4">
                          <span className="flex items-center gap-1">
                            <BedDouble size={16} /> Kapasitas: {room.capacity}
                          </span>
                        </div>
                        {room.description && (
                      ...
                      {room.priceDetails && (
                        <div className="mt-4 text-xs bg-gray-50 dark:bg-slate-700/30 p-3 rounded-lg space-y-1 mb-4 border dark:border-slate-750">
                          ...
                          {room.priceDetails.breakdown.map((day, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between text-gray-600 dark:text-gray-400 py-0.5 border-b border-gray-100 dark:border-slate-700/50 last:border-0"
                            >
                              <span className="flex items-center gap-1">
                                {day.date}
                                {day.isPeak && (
                      ...
                      <div className="pt-4 border-t dark:border-slate-700 flex items-center justify-between mt-auto">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {room.priceDetails
                              ? `total (${room.priceDetails.nights} malam)`
                              : "/ malam"}
                          </p>
                          {room.priceDetails &&
                            room.priceDetails.nights > 1 &&
                            roomPrice > 0 && (
                              <p className="text-[10px] text-gray-400 mt-0.5">
                                Rata-rata:{" "}
                                {formatPrice(
                                  Math.round(
                                    roomPrice / room.priceDetails.nights,
                                  ),
                                )}
                              </p>
                            )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl max-w-md w-full p-6">
            ...
            <p className="text-xs text-center text-red-500 mt-2">
              Tanggal yang dicoret berarti penuh/tidak tersedia.
            </p>
