...
export const ReportsFilterPanel: FC<ReportsFilterPanelProps> = ({
  ...
  <div className="grid grid-cols-1 gap-4 rounded-xl border bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:grid-cols-2 xl:grid-cols-8">
    <FilterSelect
      ...
      onChange={actions.setSelectedPropertyId}
      options={getPropertyOptions(properties)}
    />
    <FilterSelect
      label="Periode"
      value={filters.revenuePeriod}
      onChange={(value) => setRevenuePeriod(value, actions)}
      options={REPORT_REVENUE_PERIOD_OPTIONS}
    />
    <FilterSelect
      label="Status Transaksi"
      value={filters.selectedStatus}
      onChange={actions.setSelectedStatus}
    ...
    />
    <FilterSelect
      label="Urutan Transaksi"
      value={`${filters.sortBy}-${filters.sortOrder}`}
      onChange={(value) => applySort(value, actions)}
