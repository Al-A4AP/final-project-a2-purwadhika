...
export const RoomsListSection: FC<RoomsListSectionProps> = (props) => {
  if (props.loading) return <SectionLoading variant="table" label="Memuat kamar..." />;
  if (!props.rooms.length) return <RoomsEmptyState />;
  return <RoomsListView {...props} />;
};
...
const RoomsEmptyState = () => (
  <div className="rounded-2xl border border-slate-100 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <EmptyState title="Tidak ada tipe kamar" description="Tambahkan tipe kamar untuk mulai menerima pesanan dari tamu." />
  </div>
);
