...
export const TenantReviewsContent: FC<{ state: TenantReviewsState }> = ({ state }) => {
  ...
  return <ReviewsPageShell><ReviewsHeader /><TenantReviewsList state={state} />{state.reviews.length > 0 && <Pagination currentPage={state.pagination.page || 1} totalPages={state.pagination.totalPages || 1} totalItems={state.pagination.total || 0} onPageChange={state.handlePageChange} />}<TenantReviewDeleteModal state={state} /></ReviewsPageShell>;
};
