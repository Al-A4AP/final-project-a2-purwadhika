...
const RoomsPage: FC = () => {
  ...
  const [editingRoom, setEditingRoom] = useState<RoomWithPeakRates | null>(
    null,
  );
  ...
  const handleDayClick = async (date: Date) => {
    ...
    try {
      await availabilityService.setRoomAvailability(
        selectedRoomId,
        dateStr,
        newIsAvailable,
      );
      const data =
        await availabilityService.getRoomAvailability(selectedRoomId);
      setAvailabilities(data);
      toast.success("Status ketersediaan tanggal berhasil diubah");
    } catch (err: unknown) {
      const e = err as { response?: { data?: { message?: string } } };
      const errorMsg =
        e.response?.data?.message || "Gagal mengupdate ketersediaan";
      toast.error(errorMsg);
    }
  };

  const blockedDays = availabilities
