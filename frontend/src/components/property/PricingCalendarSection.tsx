...
export const PricingCalendarSection: FC<Props> = ({
  checkIn, checkOut, dateError, rooms,
  ...
  const getPriceForDate = (date: Date) => {
    ...
    for (const room of rooms) {
      ...
      const isBlocked = room.availabilities?.some((a: { is_available: boolean; date: string }) => !a.is_available && a.date.startsWith(dStr));
    ...
    if (availableRooms === 0 || minPrice === Infinity) return null;
    ...
    <div id="date-picker-section" className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border dark:border-slate-700 mb-8">
      ...
      <div className="flex justify-center overflow-x-auto bg-gray-50 dark:bg-slate-900/50 p-6 rounded-xl border border-gray-100 dark:border-slate-700/50">
        <DayPicker
          ...
          pagedNavigation
          components={{
            DayButton: (props: React.ButtonHTMLAttributes<HTMLButtonElement> & { day: { date: Date }, modifiers?: unknown }) => {
              ...
              return (
                <button {...buttonProps} className={`${buttonProps.className || ''} flex flex-col items-center justify-center h-full w-full py-1 relative`}>
                  <span className="text-sm font-medium z-10">{date.getDate()}</span>
                  {!isPast && pricing && (
                    <span className={`text-[9px] mt-0.5 whitespace-nowrap z-10 ${pricing.isPeak ? 'text-red-600 dark:text-red-400 font-bold' : 'text-gray-500 dark:text-gray-400'}`}>
                      {pricing.price >= 1000 ? `${(pricing.price / 1000)}k` : pricing.price}
                    </span>
                  )}
                  {!isPast && !pricing && (
                    <span className="text-[9px] mt-0.5 text-gray-400 z-10">Penuh</span>
                  )}
                </button>
              );
            }
          ...
          modifiersClassNames={{
            ...
            range_middle: 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400',
          }}
          styles={{
