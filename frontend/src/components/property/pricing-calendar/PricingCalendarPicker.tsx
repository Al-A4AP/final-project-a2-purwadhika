...
export const PricingCalendarPicker: FC<{ calendar: PricingCalendarState }> = ({ calendar }) => (
  <div className="flex justify-center overflow-x-auto bg-gray-50 dark:bg-slate-900/50 p-6 rounded-xl border border-gray-100 dark:border-slate-700/50">
    <DayPicker mode="range" selected={calendar.selectedRange} onSelect={calendar.handleSelect} disabled={[{ before: calendar.today }, ...calendar.unavailableDates]} numberOfMonths={getMonthCount()} pagedNavigation components={{ DayButton: (props) => <PricingDayButton {...props} calendar={calendar} /> }} modifiersClassNames={CALENDAR_MODIFIER_CLASSES} styles={CALENDAR_DAY_STYLES} />
  </div>
);
