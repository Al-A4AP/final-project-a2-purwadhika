...
const buttonClass =
  "mt-2 flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium transition disabled:opacity-60";
...
export const MidtransPaymentActions: FC<MidtransPaymentActionsProps> = (
  props,
) => {
  ...
  return (
    <div className="mt-2">
      <p className="text-xs italic text-yellow-600">
        {getNotice(props.order.status)}
      </p>
      <button
        onClick={() => props.retryMidtransPayment(props.order.id)}
        disabled={busy}
        className={midtransClass}
      >
        <CreditCard size={16} /> {busy ? "Memproses..." : "Coba lagi Midtrans"}
      </button>
      <button
        onClick={() => props.switchToManualPayment(props.order.id)}
        disabled={busy}
...
const cancelledNotice =
  "Pembayaran Midtrans dibatalkan. Anda bisa mencoba lagi jika tanggal masih tersedia.";
const waitingNotice =
  "Jika tidak diselesaikan dalam 2 jam, pesanan otomatis dibatalkan. Anda juga bisa buka ulang Midtrans atau pilih manual transfer.";
